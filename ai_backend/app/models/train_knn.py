...
# def fuzzy_knn_predict(X_train, y_train, X_test, k=7):
...
# def fuzzy_knn_predict_proba(X_train, y_train, X_test, k=7):
#     n_classes = len(np.unique(y_train))
...
# # 1. Chia train-test trên dữ liệu GỐC trước
...
# # 3. CHỈ áp dụng SMOTE lên tập huấn luyện (Train)
# # Ghi đè lại luôn biến X_train, y_train để code phía dưới của bạn không cần đổi tên biến
...
# y_pred_fuzzy = fuzzy_knn_predict(X_train_np, y_train_np, X_test_np, k=7)
...
# plt.xlabel('Predicted Label')
# plt.ylabel('True Label')
# plt.title('Confusion Matrix - Fuzzy KNN (k=7)')
# plt.show()
...
# plt.title('ROC Curve - CustomKNN vs Fuzzy KNN (k=7)')
# plt.legend()
# plt.grid(True)
# plt.show()
