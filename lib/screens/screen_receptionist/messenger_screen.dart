...
class StandardNetworkAvatar extends StatelessWidget {
  final String imageUrl;
  final double size;

  const StandardNetworkAvatar({
    super.key,
  ...
  Widget build(BuildContext context) {
    // Nếu không có link ảnh, hiển thị icon mặc định
    if (imageUrl.isEmpty) {
    ...
    return ClipOval(
      child: Image.network(
        ...
        height: size,
        fit: BoxFit.cover,
        // Cực kỳ quan trọng: Nếu bị lỗi CORS trên Web hoặc link ảnh hỏng,
        // app sẽ không bị crash mà tự động chuyển sang hiện Icon mặc định.
  ...
  // Widget hiển thị khi không có ảnh hoặc lỗi load ảnh
...
class ReceptionistMessengerScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ReceptionistMessengerScreen> createState() =>
      _ReceptionistMessengerScreenState();
}

class _ReceptionistMessengerScreenState
    extends ConsumerState<ReceptionistMessengerScreen> {
  ...
  void _handleSendTextMessage() {
    ...
    ref
        ...
        .sendMessage(text: text, roomId: _selectedRoomId!);
  ...
  Widget build(BuildContext context) {
    ...
    final currentRoom = roomsState.maybeWhen(
      data: (rooms) {
        ...
        return rooms.firstWhere((r) => r.id == _selectedRoomId,
            orElse: () => rooms[0]);
      },
      orElse: () => null,
    );
                  ...
                  Expanded(
                    child: roomsState.when(
                      data: (rooms) {
                        if (rooms.isEmpty) {
                          return Center(
                            child: Text("Không có cuộc hội thoại nào",
                                style: TextStyle(
                                    color: Colors.grey.shade400, fontSize: 13)),
                          );
                        }
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return InkWell(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  color: isSelected
                                      ? kSecondaryColor.withOpacity(0.06)
                                      : Colors.white,
                                  border: const Border(
                                ...
                                child: Row(
                                  children: [
                                    // ĐÃ SỬA: Dùng class đa nền tảng, có xử lý lỗi khi không load được ảnh
// Thêm dấu ? ngay sau currentRoom
                                    StandardNetworkAvatar(
                                      imageUrl: currentRoom?.patientAvatar ??
                      ...
                      loading: () => const Center(
                          child: CircularProgressIndicator(
                              strokeWidth: 2, color: kSecondaryColor)),
                      error: (err, _) =>
                          const Center(child: Text("Lỗi tải danh sách")),
                    ),
                  ),
                ],
              ),
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          // ĐÃ SỬA: Thêm Expanded bảo vệ header không tràn khung
                  ...
                  Expanded(
                    child: chatState.when(
                      data: (messages) {
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final isMe = currentRoom != null
                                ? (msg.senderId != currentRoom.id)
                                : msg.isMe;
                      ...
                      error: (err, _) => const Center(
                          child: Text("Không thể nạp tin nhắn phòng này.")),
                    ),
                  ),
            ...
            child: Container(
              ...
              child: roomsState.maybeWhen(
                data: (rooms) {
                  if (rooms.isEmpty || _selectedRoomId == null) {
                    return const Center(
                        child: Text("Không có hồ sơ bệnh nhân"));
                  }
                  final currentRoom = rooms.firstWhere(
                      (r) => r.id == _selectedRoomId,
                      orElse: () => rooms[0]);
                  ...
                  return SingleChildScrollView(
                    // ĐÃ SỬA: Bọc chống tràn dọc cho cột thông tin hành chính
                    child: Column(
                      children: [
...
// ĐÃ SỬA: Dùng class đa nền tảng, có xử lý lỗi khi không load được ảnh
                        ...
                        Container(
                          ...
                          child: Text(currentRoom.patientStatus ?? "Bệnh nhân",
                              style: const TextStyle(
                                  color: kSecondaryColor,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 11)),
                        ...
                        Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildInfoRow(Icons.phone_iphone, "Số điện thoại",
                                  currentRoom.patientPhone ?? 'Chưa cập nhật'),
                              _buildInfoRow(Icons.cake_outlined, "Tuổi tác",
                                  currentRoom.patientAge ?? 'Chưa rõ'),
                              _buildInfoRow(Icons.wc_rounded, "Giới tính",
                                  currentRoom.patientGender ?? 'Nam/Nữ'),
                              const SizedBox(height: 12),
                              ...
                              SizedBox(
                                ...
                                child: ElevatedButton.icon(
                                  ...
                                  label: const Text("Tạo lịch khám nhanh",
                                      style: TextStyle(
                                          color: Colors.white,
                                          fontSize: 12,
                                          fontWeight: FontWeight.bold)),
                ...
                orElse: () =>
                    const Center(child: Text("Đang tải dữ liệu hồ sơ...")),
              ),
            ),
          )
        ],
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Expanded(
            // ĐÃ SỬA: Chống tràn text bên phải khi text hành chính quá dài
            child: Text(
              value,
